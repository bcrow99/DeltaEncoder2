...
def resize_y2(src: np.ndarray, new_ydim: int) -> np.ndarray:
    ydim, xdim = src.shape
    if new_ydim == ydim:
        return src.copy()
...
def resize2(src: np.ndarray, new_xdim: int, new_ydim: int) -> np.ndarray:
    dim = src.shape
    ydim = dim[0]
    xdim = dim[1]
    
    if new_xdim * new_ydim < xdim * ydim:
        tmp = resize_x2(src, new_xdim)
        return resize_y2(tmp, new_ydim)
    else:
        tmp = resize_y2(src, new_ydim)
        return resize_x2(tmp, new_xdim)